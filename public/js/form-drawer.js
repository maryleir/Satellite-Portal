...
(function() {
    ...
    function initFormDrawer() {
        ...
        /* ═══════════════════════════════════════════
         * UPLOAD MODE — Server-rendered panel with file upload
         * ═══════════════════════════════════════════ */

        /**
         * Load the upload panel via REST (server-rendered HTML).
         * Panel includes dropzone, version history, and comments.
         */
        function loadUploadPanel(sessionId, fileType, isReadonly) {
            ...
            fetch(wsspData.restUrl + 'file-uploads?session_id=' + sessionId + '&file_type=' + encodeURIComponent(fileType), {
                headers: { 'X-WP-Nonce': wsspData.nonce }
            })
            .then(function(res) { return res.json(); })
            .then(function(data) {
                ...
                content.style.display = '';
                content.innerHTML = data.html;
                bindUploadPanelEvents(sessionId, fileType);
            })
            .catch(function(err) {
                console.error('[WSSP Drawer] Upload panel load error:', err);
                loading.style.display = 'none';
                errorEl.style.display = '';
            });
        }
        ...
        /**
         * Bind all event handlers for the server-rendered upload panel.
         */
        function bindUploadPanelEvents(sessionId, fileType) {
            var panel        = content.querySelector('.wssp-upload');
            var dropzone     = content.querySelector('#wssp-dropzone');
            var fileInput    = content.querySelector('#wssp-file-input');
            var browse       = content.querySelector('.wssp-upload__browse');
            var staged       = content.querySelector('#wssp-staged');
            var stagedName   = content.querySelector('#wssp-staged-filename');
            var stagedRemove = content.querySelector('#wssp-staged-remove');
            var stagedSubmit = content.querySelector('#wssp-staged-submit');
            var pendingFile  = null;

            if (!panel) return;

            // ─── Stage a file (show confirmation area, don't upload yet) ───
            ...
            if (stagedSubmit) {
                stagedSubmit.addEventListener('click', function() {
                    ...
                    uploadFile(sessionId, fileType, pendingFile);
                });
            }

            // ─── Status change (logistics) ───
            // ─── Show/hide change note textarea based on status selection ───
            ...
            panel.querySelectorAll('.wssp-upload__status-save').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    // Require a note when requesting changes
                    ...
                    fetch(wsspData.restUrl + 'file-uploads/' + entryId + '/status', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-WP-Nonce': wsspData.nonce
            ...
            panel.querySelectorAll('.wssp-upload__vbi-save').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    fetch(wsspData.restUrl + 'file-uploads/vbi', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-WP-Nonce': wsspData.nonce
                    ...
                    .then(function(data) {
                        if (data.success && data.html) {
                            ...
                            alert(data.message || 'Could not save selection.');
                        }
                    })
                    .catch(function() {
                        btn.disabled = false;
            ...
            panel.querySelectorAll('.wssp-upload__note-submit').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    fetch(wsspData.restUrl + 'file-uploads/' + entryId + '/comments', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-WP-Nonce': wsspData.nonce
                    ...
                    .then(function(data) {
                        if (data.success && data.html) {
                            ...
                            alert(data.message || 'Could not post note.');
                        }
                    })
                    .catch(function() {
                        btn.disabled = false;
                ...
                var input = panel.querySelector('.wssp-upload__note-add-input[data-entry-id="' + btn.getAttribute('data-entry-id') + '"]');
                if (input) {
        ...
        /**
         * Upload a file via the REST API with progress tracking.
         */
        function uploadFile(sessionId, fileType, file) {
            var dropContent  = content.querySelector('.wssp-upload__dropzone-content');
            var progressEl   = content.querySelector('.wssp-upload__progress');
            var progressFill = content.querySelector('.wssp-upload__progress-fill');
            var progressText = content.querySelector('.wssp-upload__progress-text');
            var noteInput    = content.querySelector('#wssp-upload-note');

            if (!progressEl) return;

            // Show progress, hide dropzone content
            ...
            xhr.addEventListener('load', function() {
                ...
                if (xhr.status >= 200 && xhr.status < 300 && data && data.success) {
                    ...
                    // Re-render panel with updated HTML from server
                    setTimeout(function() {
                        if (data.html) {
                            content.innerHTML = data.html;
                            bindUploadPanelEvents(sessionId, fileType);
                        } else {
                            loadUploadPanel(sessionId, fileType, currentReadonly);
                        }
                    }, 600);
                } else {
                    ...
                    // If the file was approved while the drawer was open,
                    // reload the panel to show the approved notice and refresh the dashboard
                    if (data && data.approved) {
                        ...
                        setTimeout(function() {
                            ...
                            // Re-show the staged area if it was visible
                            var staged = content.querySelector('#wssp-staged');
                            if (staged) staged.style.display = '';
                        }, 3000);
                    }
            ...
            xhr.open('POST', wsspData.restUrl + 'file-uploads');
            xhr.setRequestHeader('X-WP-Nonce', wsspData.nonce);
            xhr.send(formData);
        }
    }
})();