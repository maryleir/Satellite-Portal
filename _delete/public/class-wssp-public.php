...
class WSSP_Public {
    ...
    private function render_session_detail( $session_id, $all_sessions ) {
        ...
        // ─── Load merged session + Formidable data (needed for overview AND condition evaluator) ───
        $event_type   = $session['event_type'] ?? 'satellite';
        $session_data = $this->formidable->get_full_session_data( $session['session_key'] );

        // ─── Load session meta and compute add-on states early ───
        // Needed by the dashboard engine for accurate phase status computation.
        ...
        // ─── Set session context for shortcode resolution ───
        // All do_shortcode() calls in templates will now resolve
        // session-specific shortcodes like [assigned-room], [title], etc.
        ...
        // ─── Get enriched dashboard data from the phase engine ───
        // Pass $merged_data for condition evaluation and $addon_states for accurate completion.
        ...
        // Config & permissions
        $config       = $this->config->get_event_type( $event_type );
        $event_label  = $config['label'] ?? 'Satellite Symposium';
